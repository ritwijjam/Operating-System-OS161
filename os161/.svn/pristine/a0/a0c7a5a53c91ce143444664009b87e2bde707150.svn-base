...
//this will be used to disable and enable menu thread later on

extern struct thread* menuthread;
extern struct lock* menulock;
...
struct process{
    ...
    int ppid; //process id of the parent of this process
    ...
    int exitcode; //idk what this does but oh well gg 
    int interest; //total interests for each process
    struct lock* process_exit_lock; // lock used during cv  
    struct lock* process_exit_shit; // lock for incrementing exit stuff
    struct cv* process_wait_cv; // waiting cv 
