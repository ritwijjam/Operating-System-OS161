...
int DB_LOCORE_STATUS = 0;
int DB_SYSCALL_STATUS = 0;
int DB_INTERRUPT_STATUS = 0;
int DB_DEVICE_STATUS = 0;
int DB_THREADS_STATUS = 0;
int DB_VM_STATUS = 0;
int DB_EXEC_STATUS = 0;
int DB_VFS_STATUS = 0;
int DB_SFS_STATUS = 0;
int DB_NET_STATUS = 0;
int DB_NETFS_STATUS = 0;
int DB_KMALLOC_STATUS = 0;
...
common_prog(int nargs, char **args)
{
	...
	if (result) {
        ...
        // puts the menu to cv wait if children processes are running
        // acquires the menu lock 
        // calls menu cv to put the parent to wait if the child processes are running
        // releases the lock once menu is done
...
{	
		...
		if(!strcmp(args[1], "1")){
			...
			if(!strcmp(args[2], "on")){
				DB_INTERRUPT_STATUS = 1;
				dbflags = (dbflags|DB_INTERRUPT);
			}
			else if(!strcmp(args[2], "off"))
			{
				DB_INTERRUPT_STATUS = 0;
				dbflags = 0x000;
			}
