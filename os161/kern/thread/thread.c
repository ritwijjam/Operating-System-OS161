...
// menu shit goes here ::::::::::::((( feels bad dood
...
thread_create(const char *name)
{
	...
	if (thread->t_name==NULL) {
        ...
        thread->threadBoi = lock_create("myMans");
	...
	// them here.
        ...
        for(i = 0; i < MAX_NUMBER_OF_PROCESS; i++){
            // filling up the table of process struct 
            if (total_processes[i].busy == 0 && total_processes[i].process_thread == NULL){
                thread->pid_thread = i;
                total_processes[i].pid = i; 
                total_processes[i].process_thread = thread;
                total_processes[i].interest = 0;
                total_processes[i].busy = 1;
                total_processes[i].wait = 0;
                total_processes[i].process_exit_lock = lock_create("exitDood");
                total_processes[i].process_exit_shit = lock_create("exitMans");
                total_processes[i].process_wait_cv = cv_create("waitMans");
                user_threads ++; // NUMBER OF USER THREAD JUST INCREASED BC NEW PROCESS DOOD !!!!!!!!!!!
                break; // get outta the loop once u add the process to the list of processes
            }
        }
...
void init_all_process(void){
    // when everything starts the number of user thread = 0;
    // init to 0 bc when you call bootstrap it will automatically create a thread (read: process) to make it 0 + 1 =  1
    int i;
    for (i=0; i< MAX_NUMBER_OF_PROCESS; i++){
        // everything is free 
        total_processes[i].busy = 0;
        total_processes[i].wait = 0;
        total_processes[i].exit = 0;
        total_processes[i].exitcode = 0;
        total_processes[i].interest = 0;
        total_processes[i].process_thread = NULL; 
    }
    user_threads = 0;
}
	    ...
	    struct thread **ret)
{
	...
	//set new ppID to current threads id
	//newguy->ppID = curthread->pid_thread;
...
thread_exit(void)
{
	...
	splhigh();        
        ...
        // releases the menu thread if the current thread is menu
        
            if (curthread == menuthread){
            lock_acquire(menulock);
            cv_broadcast(menucv, menulock);
...
void
thread_random_wakeup(const void *addr)
{
	int i, result;
